...
process.env.ALLURE_RESULTS_DIR = `raw-test-data/pages/landingpages/freesample/screenshot/review/devicelarge`;

  for(const device of largeScreen){
    test.describe(`Amerisano LandingPage Page test ${device.name}`,async () => {

        test(` Freesample LP review screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
          ...
          //  const popup_pom = new DiscountCoupanPopup(page)
           ...
           // await popup_pom.closeModal();
            ...
            //review Section
           await pom.Assert_ClientReviewSection_Visible();
           ...
           const checkWidth = await isHorizontalScrollbarPresent(page);
            ...
            await testInfo.attach(`freeSample review ${device.name} viewport: ${device.width}X${device.height}`, {
