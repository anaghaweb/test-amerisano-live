...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/landingpages/buy/screenshot/as600/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano/Buy LandingPage test ${device.name}`,async () => {
        ...
        test(`AS600 screenshot ${device.name}`, async ({}, testInfo  )=>{
            ...
            await testInfo.attach(`AS600 screenshot ${device.name}`, {
