...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/landingpages/buy/screenshot/betterpricing/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano/Buy LandingPage test ${device.name}`,async () => {
        ...
        test(`better pricing screenshot ${device.name}`, async ({}, testInfo  )=>{
            ...
            await testInfo.attach(`better pricing screenshot ${device.name}`, {
