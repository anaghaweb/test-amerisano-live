...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/landingpages/buy/screenshot/as588/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano/Buy LandingPage test ${device.name}`,async () => {
        ...
        test(`AS588 screenshot ${device.name}`, async ({}, testInfo  )=>{
            ...
            await testInfo.attach(`AS588 screenshot ${device.name}`, {
