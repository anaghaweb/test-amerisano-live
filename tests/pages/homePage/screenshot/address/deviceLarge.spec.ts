...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/homepage/screenshot/address/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano HomePage test ${device.name}`,async () => {
        ...
        test(`AddressMap screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
           ...
           //Accept Cookie and close Discount Coupon
            ...
            await testInfo.attach(`Address ${device.name} viewport: ${device.width}X${device.height}`, {
