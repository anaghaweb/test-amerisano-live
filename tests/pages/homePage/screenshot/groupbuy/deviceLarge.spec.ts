...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/homepage/screenshot/groupbuy/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano HomePage test ${device.name}`,async () => {
        ...
        test(`GroupBuy screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
           ...
           //Accept Cookie and close Discount Coupon
            ...
            //Review Section
            await pom.Assert_GroupBuyingSection();
            ...
            await testInfo.attach(`Groupbuy ${device.name} viewport: ${device.width}X${device.height}`, {
