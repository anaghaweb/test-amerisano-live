...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/homepage/screenshot/product/specs/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano HomePage test ${device.name}`,async () => {
        ...
        test(`Product full specs screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
           ...
           //Accept Cookie and close Discount Coupon
            ...
            //Banner Section
            await pom.AssertFullSpecsVisible();
            ...
            await testInfo.attach(`Productfullspecs ${device.name} viewport: ${device.width}X${device.height}`, {
