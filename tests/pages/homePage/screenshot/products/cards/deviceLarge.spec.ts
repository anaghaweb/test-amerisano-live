...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/homepage/screenshot/product/cards/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano HomePage test ${device.name}`,async () => {
        ...
        test(`Product section screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
           ...
           //Accept Cookie and close Discount Coupon
            ...
            //Banner Section
            await pom.AssertProductSection();
            ...
            await testInfo.attach(`Product section ${device.name} viewport: ${device.height}x${device.width}`, {
