...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/orderpage/screenshot/needmoreinfo/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano Order Page test ${device.name}`,async () => {
        ...
        test(`Need Info Form screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
          ...
          //  const popup_pom = new DiscountCoupanPopup(page)
           ...
           await pom.Assert_NeedInfoForm_section();
           ...
           const checkWidth = await isHorizontalScrollbarPresent(page);
            ...
            await testInfo.attach(`NeedMoreInfoForm ${device.name} viewport: ${device.width}X${device.height}`, {
