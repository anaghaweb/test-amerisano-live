...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/orderpage/screenshot/betterpricing/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano Order Page test ${device.name}`,async () => {
        ...
        test(`Better Pricing form screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
          ...
          //  const popup_pom = new DiscountCoupanPopup(page)
           ...
           await pom.Assert_BetterPricing_Section_Visible();
           ...
           const checkWidth = await isHorizontalScrollbarPresent(page);
            ...
            await testInfo.attach(`Better Pricing ${device.name} viewport: ${device.width}X${device.height}`, {
