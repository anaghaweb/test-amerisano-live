...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/pages/orderpage/screenshot/as580/devicelarge";
...
test.beforeEach("teardown Context", async () => {
  ...
  for(const device of largeScreen){
    test.describe(`Amerisano Order Page test ${device.name}`,async () => {
        ...
        test(`AS580 form screenshot ${device.name} viewport: ${device.width}x${device.height}`, async ({}, testInfo  )=>{
          ...
          //  const popup_pom = new DiscountCoupanPopup(page)
           ...
           // await popup_pom.closeModal();
            ...
            //AS580 Section
           await pom.Assert_AS580_Section_Visible();
           ...
           const checkWidth = await isHorizontalScrollbarPresent(page);
            ...
            await testInfo.attach(`AS580 ${device.name} viewport: ${device.width}X${device.height}`, {
