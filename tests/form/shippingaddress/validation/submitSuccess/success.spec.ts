...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/forms/shipadd/validation/submitsuccess";
...
for (const device of largeScreen) {
  test(`Shipping Address form submit success on ${device.name} `, async ({}, testInfo) => {
    /**
   ...
   await page.waitForTimeout(4000);
    await expect(page
     ...
     await discountCoupanPopup.closeModal();
      ...
      await testInfo.attach(`shipAddress_form_submitSuccess_${device.name}.png`, {
