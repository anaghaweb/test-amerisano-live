...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/forms/paymentmethod/screenshot/creditcard/deviceLarge";
...
for (const device of largeScreen) {
  ...
  test(`CreditCard Payment Method screenshot ${device.name} `, async ({}, testInfo) => {
    /**
   ...
   await discountCoupanPopup.closeModal();
     ...
     */
      ...
      await testInfo.attach(`CreditCard_screenshot_${device.name}.png`, {
