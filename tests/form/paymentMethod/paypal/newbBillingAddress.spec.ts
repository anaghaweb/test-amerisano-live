...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/forms/paymentmethod/paypal/newbillingaddress";
...
for (const device of largeScreen) {
  ...
  test(`test should fail as billing/shipping address are not different ${device.name} `, async ({browserName}, testInfo) => {
    /**
 ...
 await checkoutBtn.ProceedToCheckOutBtn().click();
   ...
   await discountCoupanPopup.closeModal();
     ...
     */
      ...
      //Fill Card Details
      //await pm_pom.FillCardDetails();
      //add new Billing address
      await pm_pom.selectNewBillingAddress();
      await pm_pom.fillBillingAddressForm(billingAddressData);
      ...
      await testInfo.attach(`CreditCard_newBillingAddress_${device.name}.png`, {
        body: await page.screenshot(),
        contentType: "image/png",
      });     
      
      test.fail(browserName===`${device.name}`,"Billing address does not change")
  });
}
