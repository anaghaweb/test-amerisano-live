import { test, expect } from "@playwright/test";
import { largeScreen } from "../../../../../contexts";
import {
  setupTest,
  teardownTest,
  getPage,
} from "../../../../../testSetup/setupTest";
import { Cookies } from "../../../../../Objects/Shared";
import {  GotoLandingPage } from "../../../../../Objects/Pages";
import {FreeSampleForm} from "../../../../../Objects/forms";

const testdata = {
  firstname: "john",
  lastname: "doe",
  email: "john@example.com",
  address1: "1, lakeview",
  address2: "abc town",
  city: "future city",
  zipcode: "12345",
  phone: "1234567890",
  company: "future company",
};
test.beforeEach(async () => {
  await teardownTest();
});
//test results storage path
process.env.ALLURE_RESULTS_DIR =
  "raw-test-data/forms/freesample/validation/feedback";

for (const device of largeScreen) {
  test.describe(`${device.name} confirm feedback validation`, () => {
    test(`should not submit form without feedback option selected ${device.name}`, async ({}, testInfo) => {
      await setupTest({ device });
      const page = await getPage();
      const nw_pom = new GotoLandingPage(page);
      const cookie_pom = new Cookies(page);
      //go to landing page
      await nw_pom.LoadLandingPage();
      //create new instance of form
      const fs_pom = new FreeSampleForm(page);
      await expect(fs_pom.as580ChooseSize()).toBeVisible();
      await fs_pom.chooseSizeAS580();
      await fs_pom.fillGeneralInfo(testdata);
      await fs_pom.fillState();
      await fs_pom.submitInfo();
      await fs_pom.feedbackOption_1();
      //await fs_pom.willGiveFeedback();
      await fs_pom.sendFreeSample();
      await testInfo.attach(`warningtext_${device.name}.png`, {
        body: await page.screenshot(),
        contentType: "image/png",
      });
    });
  });
}
