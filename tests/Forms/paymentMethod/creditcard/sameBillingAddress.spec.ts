...
process.env.ALLURE_RESULTS_DIR = "raw-test-data/forms/paymentmethod/creditcard/samebillingaddress";
  ...
  test(`billing & shipping address should be same ${device.name} `, async ({}, testInfo) => {
   ...
   await discountCoupanPopup.closeModal();     
     ...
     */
      ...
      await testInfo.attach(`CreditCard_sameBillingAddress_${device.name}.png`, {
